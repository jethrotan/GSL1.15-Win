...
rk4imp_alloc (size_t dim)
{
  ...
  state->esol = (modnewton1_state_t*)modnewton1_alloc (dim, RK4IMP_STAGE);
              ...
              const gsl_odeiv2_system * sys)
{
  ...
  double c[2];
  c[0]= (3.0 - sqrt (3.0))/6.0;
  c[1] = (3.0 + sqrt (3.0))/6.0;

  gsl_matrix_set (A, 0, 0, 1.0 / 4);
  gsl_matrix_set (A, 0, 1, (3 - 2 * sqrt ((double)3)) / 12);
  gsl_matrix_set (A, 1, 0, (3 + 2 * sqrt ((double)3)) / 12);
