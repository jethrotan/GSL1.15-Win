...
hyperg_1F1_ab_posint(const int a, const int b, const double x, gsl_sf_result * result)
{
  ...
  if(a == b) {
    ...
    result->val = Ma/Mn;
    result->err = 2.0 * GSL_DBL_EPSILON * (fabs((double)a) + 1.0) * fabs(Ma/Mn);
    return stat_CF1;
  }
    ...
    result->err += 4.0 * GSL_DBL_EPSILON * (fabs((double)b-a)+1.0) * fabs(result->val);
    ...
    if(b < a) {
      ...
      if(x + log(fabs(x/b)) < GSL_LOG_DBL_MAX-2.0) {
        ...
        result->err *= fabs((double)a-b)+1.0;
        return GSL_SUCCESS;
      }
      else {
      ...
      result->val  = Mn;
      result->err  = fabs(Mn) * (1.0 + fabs((double)a)) * fabs(r_Mn.err / r_Mn.val);
    ...
    if(a <= 0.5*(b-x) || a >= -x) {
      ...
      result->err *= fabs((double)b-a)+1.0;
      return GSL_SUCCESS;
    }
    else {
...
hyperg_1F1_a_negint_poly(const int a, const double b, const double x, gsl_sf_result * result)
{
  if(a == 0) {
    ...
    result->val = poly;
    result->err = 2.0 * (sqrt((double)N) + 1.0) * GSL_DBL_EPSILON * fabs(poly);
    return GSL_SUCCESS;
  }
}
...
gsl_sf_hyperg_1F1_int_e(const int a, const int b, const double x, gsl_sf_result * result)
{
  ...
  else if(x > 100.0  && GSL_MAX_DBL(1.0,fabs((double)b-a))*GSL_MAX_DBL(1.0,fabs((double)1-a)) < 0.5 * x) {
    /* x -> +Inf asymptotic */
    return hyperg_1F1_asymp_posx(a, b, x, result);
  }
  else if(x < -100.0 && GSL_MAX_DBL(1.0,fabs((double)a))*GSL_MAX_DBL(1.0,fabs((double)1+a-b)) < 0.5 * fabs(x)) {
    /* x -> -Inf asymptotic */
    return hyperg_1F1_asymp_negx(a, b, x, result);
  }
  else if(a < 0 && b < 0) {
