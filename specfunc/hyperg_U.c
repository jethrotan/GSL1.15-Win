                    ...
                    gsl_sf_result * result)
{
  ...
  if(N <= 0) {
    ...
    result->err += fabs(poch.val) * (fabs((double)N) + 2.0) * GSL_DBL_EPSILON * fabs(sum_val);
    result->err += 2.0 * GSL_DBL_EPSILON * fabs(result->val);
                  ...
                  gsl_sf_result_e10 * result)
{
  if(a == 0) {
    ...
    result->val  = -b + x;
    result->err  = 2.0 * GSL_DBL_EPSILON * (fabs((double)b) + fabs(x));
    result->err += 2.0 * GSL_DBL_EPSILON * fabs(result->val);
    result->e10  = 0;
    return GSL_SUCCESS;
  }
  ...
  else if(ASYMP_EVAL_OK((double)a,b,x)) {
  ...
  else if(SERIES_EVAL_OK((double)a,b,x) && 1 + a - b > 0) {
    gsl_sf_result ser;
    ...
    y.val = Ua;
    y.err = 4.0 * GSL_DBL_EPSILON * (fabs((double)a)+1.0) * fabs(Ua);
    ...
    y.err += 2.0 * GSL_DBL_EPSILON * (fabs((double)a) + 1.0) * fabs(Ua);
    ...
    if(b <= x) {
      ...
      if(Ua == 0.0) {
        ...
        double oUa_err   = 2.0 * (fabs((double)a_target-a) + CF1_count + 1.0) * GSL_DBL_EPSILON * fabs(1.0/Ua);
...

      {
        ...
        Ua1_bck_val = Ua;
        Ua1_bck_err = 2.0 * GSL_DBL_EPSILON * (fabs((double)a1-a)+CF1_count+1.0) * fabs(Ua);
        stat_bck = stat_CF1;
      }
      ...
      if(b == 2*a1 && a1 > 1) {
        ...
        Ua1_for_err += 2.0 * GSL_DBL_EPSILON * (fabs((double)a1-a0)+1.0) * fabs(Ua1_for_val);
      }
              ...
              gsl_sf_result_e10 * result)
{
  ...
  if(a == 0.0) {
    ...
    if(b <= x) {
      ...
      lnm.err = 2.0 * GSL_DBL_EPSILON * (fabs(lm_for) + fabs((double)scale_count_for - scale_count_bck)*fabs(lnscale));
                             ...
                             gsl_sf_result_e10 * result)
{
  ...
  if(x == 0.0 && b >= 1) {
    ...
    if(b >= 1) {
      ...
      double ln_pre_err = 2.0 * GSL_DBL_EPSILON * (fabs((double)b)+1.0) * fabs(ln_x);
